...
class TableRead:
    ...
    def tables_read(self,class_name):   #찾을 클래스 이름
        self.tables = self.soup.find_all('table',{'class':class_name}) #해당 클래스이름으로 테이블태그 전부 찾기

    def trs_read(self,num): #몇번째 테이블
        self.trs = self.tables[num].find_all('tr')  #몇번째(찾는) 테이블에서 tr코드 전부 찾기
        for i in range(len(self.trs)):
            self.table1_tr_list.append(self.trs[i].text.strip())    #html코드 버리고 텍스트 부분만 추출
